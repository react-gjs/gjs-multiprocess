type FirstLetter<T extends string> = T extends `${infer F}${infer R}`
  ? F
  : never;
...
type IsExportKey<O, K> = K extends string
  ? O extends Record<K, Function>
...
type AsAsync<F> = F extends (...args: infer A) => infer R
  ? (...args: A) => Promise<R>
  : never;
...
export type InterfaceOf<I extends object> = {
  [K in keyof I as IsExportKey<I, K> extends true
    ? Concat<K, "Async">
    : never]: AsAsync<I[K]>;
} & {
  [K in keyof I as IsExportKey<I, K> extends true
